...
from vehicletracker.models.dwell_time import DwellTimeModel

_LOGGER = logging.getLogger(__name__)

class HaWeeklyStopDwell(DwellTimeModel):
    ...
    def train(self, time : datetime, spatial_ref : SpatialRef, parameters : Dict[str, Any]) -> Dict[str, Any]:
        ...
        data = pd.read_csv(f'./cache/weekly_average_stop_dwell_time_{self.freq}.csv.gz')
        data['time'] = pd.to_datetime('1970-01-01') + (data['dow'] - 1) * pd.to_timedelta('1D') + pd.to_timedelta(data['tod'])
        data_pivot = data.groupby(['time', 'stop_ref'])['stop_dwell_time'].first().unstack(1)
    ...
    def predict(self, predict_params):        
        ...
        if 'time' in predict_params and isinstance(predict_params['time'], str):
            if not predict_params['stopPointRef'] in self.travel_time_lookup.columns:
                return None
            ...
            return self.travel_time_lookup.loc[time_lookup, predict_params['stopPointRef']]
        # Batch prediction
