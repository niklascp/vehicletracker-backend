...
def callback(ch, method, properties, body):
    if properties.content_type == 'application/json':
        ...
        _LOGGER.error(f"Unknown content type: {properties.content_type }.")
...
def main():
    ...
    channel.queue_declare(queue=WORKER_QUEUE_NAME, auto_delete=True)

    for event_type in LISTEN_EVENT_TYPES:
        channel.queue_bind(exchange=EVENTS_EXCHANGE_NAME, queue=WORKER_QUEUE_NAME, routing_key=event_type)
